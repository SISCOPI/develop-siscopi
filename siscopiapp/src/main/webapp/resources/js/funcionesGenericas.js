...
//	 var fh= f.getDate()+"/"+(f.getMonth() +1)+"/"+f.getFullYear();
//	 
//	 document.getElementById("fechahoy").innerHTML = fh;
//}
...
//    alert("mostrar:"+$(obj).attr('id'));
    if ($(obj).attr('id').indexOf('imgaddentre') !== -1)
    {
        $("#entregable").show();
        $("#imgaddentre").hide();
    }
    if ($(obj).attr('id').indexOf('imgaddentreindividual') !== -1)
    {
        $('#entregables').show();
        var entrega = $('#formPropuesta\\:nentregable').val();
        var numero = 1;
        $('#tentregables').append('<tr><td class="text-center">' + numero + '</td><td>' + entrega + '</td></tr>');
        $('#formPropuesta\\:nentregable').val("");
        $('#formPropuesta\\:nentregable').focus();
    }
    if ($(obj).attr('id').indexOf('imgaddacti') !== -1)
    
    {
        $("#actividad").show();
        $("#imgaddacti").hide();
    }
    if ($(obj).attr('id').indexOf('imgaddactiindividual') !== -1)
    {
        $('#actividades').show();
